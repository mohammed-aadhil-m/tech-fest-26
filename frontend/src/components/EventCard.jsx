    ...
    >
      ...
      >
        ...
        <div className="w-full h-48 sm:h-56 bg-[#0a0a0c] relative overflow-hidden flex-shrink-0 border-b border-white/10" style={{ transform: 'translateZ(10px)' }}>
          ...
          {/* Registration count badge (Bottom Right) */}
          {!isComingSoon && (
            <div className="absolute bottom-4 right-4 z-20" style={{ transform: 'translateZ(30px)' }}>
              <span
                className="inline-flex items-center gap-1.5 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded-md bg-black/75 text-gray-200 border border-white/20 backdrop-blur-md shadow-md group-hover:border-red-500/50 group-hover:text-white transition-all duration-300"
                title={`${event.isTeamEvent ? (event.teamCount ?? event.registeredCount ?? 0) + ' team(s)' : (event.registrationCount ?? event.registeredCount ?? 0) + ' individual(s)'} registered`}
              >
                {event.isTeamEvent ? (
                  <>
                    <Users size={11} className="text-red-400" />
                    <span>
                      {event.teamCount ?? event.registeredCount ?? 0} {(event.teamCount ?? event.registeredCount ?? 0) === 1 ? 'Team' : 'Teams'}
                    </span>
                  </>
                ) : (
                  <>
                    ...
                    <span>
                      {event.registrationCount ?? event.registeredCount ?? 0} {(event.registrationCount ?? event.registeredCount ?? 0) === 1 ? 'Individual' : 'Individuals'}
                    </span>
                  </>
                )}
              </span>
