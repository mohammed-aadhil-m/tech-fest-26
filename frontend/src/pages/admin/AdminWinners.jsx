  ...
  return (
    <div className="p-6 md:p-8 relative z-10">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-display font-black text-white tracking-wide">Hall of <span className="text-red-500">Champions</span></h1>
          <p className="text-sm text-gray-400 mt-1 uppercase tracking-wider font-bold">Manage TECH FEST '26 victors</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-black/60 border border-white/10 rounded-2xl backdrop-blur-md shadow-[0_0_15px_rgba(0,0,0,0.5)]">
          ...
          <p className="font-bold text-white uppercase tracking-widest">No victors declared yet</p>
          <button onClick={() => setAddModal(true)} className="mt-4 bg-white/5 hover:bg-white/10 text-gray-300 font-bold uppercase tracking-widest text-xs py-2 px-6 rounded-xl border border-white/10 transition-all">Declare First Victor</button>
        </div>
      ) : (
      ...
      {/* Add Winner Modal */}
      <Modal isOpen={addModal} onClose={() => setAddModal(false)} title="Declare Victor" size="md">
        <form onSubmit={handleSubmit} className="p-6 space-y-5 bg-[#050505]">
          <div>
            <label className="block text-[10px] font-bold text-red-500 uppercase tracking-widest mb-2">Target Event *</label>
          ...
          <div>
            <label className="block text-[10px] font-bold text-red-500 uppercase tracking-widest mb-2">Rank Placement *</label>
          ...
          <div>
            <label className="block text-[10px] font-bold text-red-500 uppercase tracking-widest mb-2">Participant / Squad Name *</label>
            <input type="text" className="w-full bg-black/60 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-600 focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 transition-all" placeholder="Enter victor identity" value={form.participantName}
              onChange={e => setForm(f => ({ ...f, participantName: e.target.value }))} required />
          </div>
          <div>
            <label className="block text-[10px] font-bold text-red-500 uppercase tracking-widest mb-2">Squad Identity (If Applicable)</label>
            <input type="text" className="w-full bg-black/60 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-600 focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 transition-all" placeholder="Enter squad designation" value={form.teamName}
              onChange={e => setForm(f => ({ ...f, teamName: e.target.value }))} />
          </div>
          <div>
            <label className="block text-[10px] font-bold text-red-500 uppercase tracking-widest mb-2">Institution Origin</label>
            <input type="text" className="w-full bg-black/60 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-600 focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 transition-all" placeholder="Enter institution name" value={form.college}
              onChange={e => setForm(f => ({ ...f, college: e.target.value }))} />
          </div>
          <div>
            <label className="block text-[10px] font-bold text-red-500 uppercase tracking-widest mb-2">Visual Record (Optional)</label>
            <label className="flex items-center gap-4 border border-dashed border-white/20 bg-white/5 rounded-xl p-5 cursor-pointer hover:border-red-500/50 hover:bg-red-500/5 transition-all group">
              ...
              <span className="text-xs text-gray-400 font-mono break-all">{photo ? photo.name : 'Select file to upload'}</span>
              <input type="file" accept="image/*" className="hidden" onChange={e => setPhoto(e.target.files[0])} />
            </label>
          </div>
          <div className="flex gap-4 pt-4 border-t border-white/10">
            <button type="button" onClick={() => setAddModal(false)} className="px-6 py-3 text-xs font-bold uppercase tracking-widest text-gray-400 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 hover:text-white transition-all flex-1">Abort</button>
            <button type="submit" disabled={saving} className="bg-red-600 hover:bg-red-500 text-white font-bold uppercase tracking-widest text-xs py-3 px-6 rounded-xl transition-all duration-300 shadow-[0_0_20px_rgba(220,38,38,0.4)] flex-1 flex justify-center items-center disabled:opacity-50">
              {saving ? <LoadingSpinner size="sm" /> : 'Execute Declaration'}
            </button>
          </div>
        </form>
      </Modal>
      ...
      <ConfirmationDialog
        ...
        loading={deleting}
        title="Revoke Victory Status"
        message={`Remove "${deleteTarget?.participantName}" from the Hall of Champions?`}
        confirmLabel="Execute Revocation"
        variant="danger"
      />
    </div>
  );
