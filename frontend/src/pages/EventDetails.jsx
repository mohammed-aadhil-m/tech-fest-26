               ...
               >
                ...
                {!isComingSoon && (
                  <div className="absolute bottom-4 right-4 z-20">
                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold uppercase tracking-wider rounded-lg bg-black/75 text-gray-200 border border-white/20 backdrop-blur-md shadow-md">
                      {event.isTeamEvent ? (
                        <>
                          <Users size={13} className="text-red-400" />
                          <span>{event.teamCount ?? event.registeredCount ?? 0} {(event.teamCount ?? event.registeredCount ?? 0) === 1 ? 'Team' : 'Teams'} Registered</span>
                        </>
                      ) : (
                        <>
                          <User size={13} className="text-red-400" />
                          <span>{event.registrationCount ?? event.registeredCount ?? 0} {(event.registrationCount ?? event.registeredCount ?? 0) === 1 ? 'Individual' : 'Individuals'} Registered</span>
                        </>
                      )}
                    </span>
                  </div>
