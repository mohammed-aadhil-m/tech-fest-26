...
async function updateEvents() {
  try {
    ...
    await Event.updateOne({ slug: 'bug-buster' }, { isTeamEvent: false, minTeamSize: 1, maxTeamSize: 1 });
    console.log('Updated bug-buster');

    // 3. Dev & Deploy (Individual)
    await Event.updateOne({ slug: 'dev-deploy' }, { isTeamEvent: false, minTeamSize: 1, maxTeamSize: 1 });
    console.log('Updated dev-deploy');

    // 4. Remaining Events (Team of 2)
    const remainingSlugs = ['treasure-hunt', 'connect-sketch', 'adaptune'];
    for (const slug of remainingSlugs) {
      await Event.updateOne({ slug }, { isTeamEvent: true, minTeamSize: 1, maxTeamSize: 2 });
